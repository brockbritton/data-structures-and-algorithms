...
const count = (target, input) => {
  ...
  return input.reduce((count, array) => {
    return count + array.filter(item => item === target).length;
  }, 0);
};
...
const totalSum = (input) => {
  ...
  return input.reduce((totalSum, array) => {
    return totalSum + array.reduce((sum, num) => sum + num, 0);
}, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  return input.map(subArray => {
    return subArray.filter(num => typeof num === 'number' && num % 5 === 0).map(num => Math.pow(2, num));
  });
};
